From the collaborative filtering section, we have seen that the user-item matrix is the key part of the whole algorithm. However, we have also seen the main problems in working with it, namely its huge size and its sparsity. In general, it is unfeasible to store the entirety of the matrix unless we use some data structure to represent it. In our case, we utilized a sparse representation implemented via scipy. 

The idea behind this section is to be able to represent the entire matrix in a different manner. Instead we look to approximate the user-item matrix as the product of 2 much smaller matrices. This is also called UV decomposition, as U and V are usual names for the 2 matrices, but we will use different notation following the course from LazyProgrammer.
$$\hat{R}=WU^T$$
...
Note that if $K=M$ then this factorization is exact and is not an approximation. However, by reducing $K$ we are using a dimensionality reduction technique and we encourage the model to learn the most important features to generate the user-item matrix. Note that W is usually referred to as the user matrix while U is the item matrix.
# Training
How can we ensure a good approximation? 
First we would want to define a loss function as always. Since this is a regression problem, the mean squared error is appropriate. However, since the denominator is simply the size, so it is a constant. We can drop it and simply use the squared error as our loss (for derivation purposes, for actual analysis, it is better to use mean squared error.). Again the goal is to minimize the loss, so differentiate and set to 0. 

Let $L$ be the squared error, i.e. the loss. $\Omega$ being the set of all users and items.
$$L=\sum_{i,j\in\Omega}(r_{ij}-\hat{r}_{ij})^2=\sum_{i,j\in\Omega}(r_{ij}-w_i^Tu_j)^2$$
$$\frac{\partial L}{\partial w_i}=2\sum_{j\in \Psi_i}(r_{ij}-w_i^Tu_j)(-u_j)=0$$
